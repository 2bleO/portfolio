...
const storageMet = {
  retrieve() {
    formData.storedJson = localStorage.getItem('portfolio');
    if (formData.storedJson !== null) {
      formData.storedObj = JSON.parse(formData.storedJson);
    }
  },

  prePop() {
    if (formData.storedObj !== null) {
  ...
  store() {
    ...
    if (formData.storedJson !== formData.toStore) {
      window.localStorage.setItem('portfolio', formData.toStore);
    }
  },
};
